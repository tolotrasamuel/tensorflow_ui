...
class Retrain:
    def __init__(self, params):
        global image_dir, output_graph, output_labels, model_dir, bottleneck_dir, base_trained_model_folder, saved_model_dir
        trained_model_dir = join(base_trained_model_folder, secure_filename(params["title"]))
        image_dir = params['image_dir']
        bottleneck_dir = join(trained_model_dir, 'bottleneck')
        saved_model_dir = join(saved_model_dir,  secure_filename(params["title"])+str(time.time())+'-/saved_models/1/')
        output_labels = join(trained_model_dir, 'output_labels.txt')
        print('bottle neck dir is ' + bottleneck_dir)
        output_graph = join(trained_model_dir, 'output_graph.pb')
        pass
    ...
    def retrain(self):
        ...
        # See if the command-line mean we're applying any distortions.
        ...
        with tf.Session(graph=graph) as sess:
            ...
            export_model(sess, architecture, saved_model_dir)


print("Initiating trainer app")
base_trained_model_folder = 'app/ai/trained_model'
model_dir = 'app/ai/trained_model/inceptionV3'
